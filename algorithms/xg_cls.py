...
def word2vec_lr_test():
    ...
    for i in range(0, 10):
        ...
        train_y_pred = xg_model.predict(train_X)
        test_y_pred = xg_model.predict(test_X)
        train_score = accuracy_score(train_y_pred, train_y)
        score = accuracy_score(test_y_pred, test_y)
        print("TRAIN", i, 'Accuracy: ', train_score)
        print("TEST", i, "Accuracy：", score)
        sum += score
    print("Logistic Regression Accuracy：", sum / 10)
    print(test_y[:10])
    print(xg_model.predict(test_X)[:10])
