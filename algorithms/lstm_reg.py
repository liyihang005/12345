...
def word2vec_lr_test():
    ...
    res_array, run_type = over_samples.fit_resample(res_array, run_type)


    out_type = deepcopy(run_type)
    ...
    for i in range(0, 10):
        ...
        model_pre.add(LSTM(50, activation='relu', input_shape=(train_X.shape[1], train_X.shape[2]), return_sequences=True))
        # model_pre.add(LSTM(50, activation='relu', input_shape=(train_X.shape[0], train_X.shape[1])))
        # model_pre.add(Dense(1))
        model_pre.add(Dense(len(set(out_type)), Activation('softmax')))
